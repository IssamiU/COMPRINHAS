...
export default function SupermarketsMapScreen({ navigation }: any) {
  ...
  async function loadData() {
    try {
      ...
      if (status !== "granted") { setError("permission"); setLoading(false); return; }
      ...
      if (!auth) { setError("network"); return; }
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      {!loading && error === "permission" && (
        <View style={styles.center}>
          ...
          <Text style={styles.errorText}>Permita o acesso à localização para encontrar supermercados próximos a você.</Text>
      ...
      {!loading && error === "network" && (
        <View style={styles.center}>
          ...
          <Text style={styles.errorText}>Não foi possível buscar os supermercados. Verifique sua conexão e tente novamente.</Text>
      ...
      {!loading && !error && location && (
        <View style={styles.content}>
          <MapView
            ref={mapRef}
            style={styles.map}
            initialRegion={{ latitude: location.lat, longitude: location.lng, latitudeDelta: 0.045, longitudeDelta: 0.045 }}
            showsUserLocation
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.marketInfo}>
                    <Text style={[styles.marketName, active && styles.marketNameActive]} numberOfLines={1}>{item.name}</Text>
                    {subtitle ? (
                      <Text style={[styles.marketAddress, active && styles.marketAddressActive]} numberOfLines={1}>{subtitle}</Text>
                    ) : null}
                  </View>
                  <Text style={[styles.marketDist, active && styles.marketDistActive]}>{formatDist(dist)}</Text>
                </Pressable>
              );
            }}
          />
