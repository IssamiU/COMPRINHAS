...
export default function BarcodeScannerScreen({ navigation, route }: any) {
  ...
  // useRef como lock — não tem problema de stale closure como useState
  ...
  async function handleBarcode({ data }: { data: string }) {
    // lock via ref — imune a stale closure (useState capturaria valor antigo)
  ...
  return (
    <View style={styles.container}>
      ...
      {/* hint abaixo do frame — só quando ocioso */}
      {scanState === "idle" && (
      ...
      {/* painel de resultado — substitui Alert para evitar empilhamento */}
      {showPanel && (
        <View style={styles.panel}>
          {scanState === "loading" && (
            <>
              <ActivityIndicator color={colors.primary} size="large" />
              <Text style={styles.panelTitle}>Consultando produto...</Text>
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  // painel de resultado (substitui Alert)
  panel: {
