...
export default function CreateRecipeScreen({ navigation, route }: any) {
  ...
  const [imageUrl,  setImageUrl]  = useState<string>(prefill?.imageUrl ?? "");
  ...
  const [ingredients, setIngredients] = useState<Ingredient[]>(
    prefill?.ingredients?.length
      ? prefill.ingredients.map((i: any) => ({ id: uid(), qty: String(i.quantity ?? ""), unit: i.unit ?? UNITS[0], name: i.name ?? "" }))
  ...
  async function handleSave() {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/recipes`, {
        ...
        body: JSON.stringify({
          ...
          ingredients: validIngredients.map((i, idx) => ({
            ...
            name: i.name, quantity: Number(i.qty), unit: i.unit,
          })),
          steps: validSteps.map((s, idx) => ({
            id: `${Date.now()}-step-${idx}`,
            description: s.text, hasTimer: s.hasTimer, duration: null,
          })),
      ...
      Alert.alert("Sucesso", "Receita criada!", [{ text: "OK", onPress: () => navigation.goBack() }]);
    } catch (e: any) {
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            {ingredients.map((ing, idx) => (
              <View key={ing.id} style={[styles.ingredientItem, idx < ingredients.length - 1 && styles.itemDivider]}>
                <View style={styles.ingredientGrid}>
                  <TextInput style={[styles.input, { flex: 0.9 }]} placeholder="Qtd" placeholderTextColor={colors.textMuted} keyboardType="numeric" value={ing.qty} onChangeText={(v) => updateIngredient(ing.id, { qty: v })} />
                  <Pressable style={[styles.input, styles.selectInput, { flex: 1.4 }]} onPress={() => setUnitPickerFor(ing.id)}>
                    <Text style={[styles.selectText, !ing.unit && { color: colors.textMuted }]} numberOfLines={1}>{ing.unit || "Unidade"}</Text>
                ...
                <TextInput style={[styles.input, { marginTop: 8 }]} placeholder="Nome do ingrediente" placeholderTextColor={colors.textMuted} value={ing.name} onChangeText={(v) => updateIngredient(ing.id, { name: v })} />
              </View>
            ))}
          </View>
          ...
          {steps.map((step, idx) => (
            <View key={step.id} style={[styles.card, { marginBottom: 10 }]}>
              ...
              <TextInput style={[styles.input, styles.textarea]} placeholder="Descreva o passo..." placeholderTextColor={colors.textMuted} value={step.text} onChangeText={(v) => updateStep(step.id, { text: v })} multiline textAlignVertical="top" />
              <View style={styles.timerToggleRow}>
                ...
                <Switch value={step.hasTimer} onValueChange={(v) => updateStep(step.id, { hasTimer: v })} trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#fff" />
              </View>
            </View>
          ))}
      ...
      <Modal visible={!!unitPickerFor} transparent animationType="slide" onRequestClose={() => setUnitPickerFor(null)}>
        ...
        <View style={styles.modalSheet}>
          ...
          <ScrollView style={{ maxHeight: 360 }} showsVerticalScrollIndicator={false}>
            {UNITS.map((u) => (
              <Pressable key={u} style={styles.modalOption} onPress={() => { if (unitPickerFor) updateIngredient(unitPickerFor, { unit: u }); setUnitPickerFor(null); }}>
